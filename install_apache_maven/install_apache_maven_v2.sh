...
# Usage Instructions
usage() {
    echo "Usage: $0 [-v <maven_version>] [-d <installation_directory>]"
    echo ""
    echo "Options:"
    echo "  -v <maven_version>          Specify the Maven version to install (e.g., 3.6.3). If omitted, the script fetches the latest version."
    echo "  -d <installation_directory> Specify the installation directory. Defaults based on OS."
    echo ""
    echo "Example:"
    echo "  $0 -v 3.6.3 -d /usr/local/apache-maven"
    echo "This will install Maven 3.6.3 to the specified directory."
    exit 1
}
...
# Function to fetch the latest Maven version from Apache
function fetch_latest_maven_version {
    local maven_url="https://dlcdn.apache.org/maven/maven-3/"
    local latest_version=$(curl -s $maven_url | grep -oP 'href="[\d\.]+/' | cut -d '"' -f 2 | tr -d '/' | sort -V | tail -1)

    if [[ -z "$latest_version" ]]; then
        echo "Failed to fetch the latest Maven version." >&2
        return 1
    else
        echo "$latest_version"
    fi
...
function initialize_environment {
    # Initialize MAVEN_VERSION if not specified
    if [[ -z "$MAVEN_VERSION" ]]; then
        MAVEN_VERSION=$(fetch_latest_maven_version)
        check_success "Successfully fetched the latest Maven version." "Failed to fetch the latest Maven version."
    fi
